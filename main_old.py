...
def ShareWithCondition(price,all_price_data):
    ...
    orderBase = all_price_data['high'][1] / 1000
    logging.info(" 基础比较价格= %s",orderBase)
    #   是否出现粘合的List
    repeat_list = []
    # 出现粘合的日期列表
    glue_list = []
    for i in range(0, dayAgo):
        # 日期
        timedate = getAgoDate(i)
        timestamp = time.mktime(timedate.timetuple())
        # timeStr = time.strftime("%a %Y-%m-%d", timedate.timetuple()) 输出周末
        timeStr = time.strftime("%Y-%m-%d", timedate.timetuple())
        green = moveAverage(price, timedate, 3, 5)
        red = moveAverage(price, timedate, 5, 8)
        blue = moveAverage(price, timedate, 8, 13)
...
def getGlueDate(l):
    ...
    map = {}
    for i in l:
        nextdata = getNextDate(i, 1)
        next2data = getNextDate(i, 2)
        if nextdata in l and next2data in l and nextdata != next2data:
            map[index] = [i, nextdata, next2data]
            index += 1
...
#     获取最后三个取出进场点和止损位置
    ...
    for p in priceList[3:].iterrows():
        ...
        if status == 1:
            # 红线状态
            red = getRedVal(price_flag, dataStr)
            # 是否有平半仓的机会
    ...
    # 股票代码    出现粘合日期    订单状态    进场日期    是否有平半仓    进场价格    出场日期    出场价格     利润
    report = (coreid, startTime, statusMap.get(status), intoTime, isHalf, intoPoint, outofTime, outofPrice,profit)
...
def func(i):
    while l:
        ...
        try:
            ...
            timeflagList = getGlueDate(glue_list)
            ...
            for val in timeflagList.values():
                ...
                for punit in pricedata.iterrows():
                    ...
                    # redVal = '%.3f' % getRedVal(price, punit[0])
                    # red_unit_map["index"] = redVal
